...
def process_text(text: str) -> dict:
    ...
    for chunk in doc.noun_chunks:
        res["noun_chunks"].append([str(chunk.text), str(chunk.root), spacy.explain(chunk.root.dep_)])
    res["named_entities"] = []
    for ent in doc.ents:
        res["named_entities"].append([str(ent.text), spacy.explain(ent.label_)])
    res["all_def"] = []
    for token in doc:
        if len(wordnet.synsets(token.text))!=0 and token.text not in stopwords.words():
         res["all_def"].append([token.text, wordnet.synsets(token.text)[0].definition()])
    return res
